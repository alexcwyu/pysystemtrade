...
- **Build backend**: `hatchling` (was: `hatchling` -- retained, configuration updated)
- **PEP 621 metadata**: All project metadata in `[project]` table
- **Dependencies**: Managed by `uv`, lockfile in `uv.lock`
- **Dev dependencies**: Moved from `[project.optional-dependencies]` to `[dependency-groups]` (PEP 735)
- **arctic extra**: Removed (incompatible -- arctic requires `pymongo<4`, project requires `pymongo>=4.15.4`)

### 2. Removed Legacy Files
The following files were removed (upstream may re-add them on sync):
- `MANIFEST.in` -- sdist configuration now in `[tool.hatch.build.targets.sdist]`

If these reappear after a sync, delete them again. All configuration is in `pyproject.toml`.

### 3. Source Layout
- **Layout**: `src/` layout
- **Packages moved**: `sysbrokers/`, `syscontrol/`, `syscore/`, `sysdata/`, `sysexecution/`, `sysinit/`, `syslogdiag/`, `syslogging/`, `sysobjects/`, `sysproduction/`, `sysquant/`, `systems/`, `data/` -> `src/`
- **Imports unchanged**: `import syscore`, `import systems`, etc. still work

If upstream adds files to the old location, move them to `src/`.
...
known-first-party = ["sysbrokers", "syscontrol", "syscore", "sysdata", "sysexecution", "sysinit", "syslogdiag", "syslogging", "sysobjects", "sysproduction", "sysquant", "systems"]
```
...
3. **Check source layout**: If upstream adds new modules to the old path, move them to `src/`.
4. **Re-lock**: Run `uv lock` to update `uv.lock` with any new/changed dependencies.
5. **Verify**: Run `uv sync && uv run python -c "import syscore" && uv run pytest` (if tests exist).

## Quick Commands

```bash
uv sync                                    # Install all deps
uv run python -c "import syscore"          # Verify import
